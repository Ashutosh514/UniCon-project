...
export default function AdminDashboard() {
  ...
  const fetchPending = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/moderation/pending`, {
        headers: {
  ...
  const fetchPendingPosts = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/postreviews/pending`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
  ...
  const takeAction = async (reviewId, action) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/moderation/review/${reviewId}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const takePostAction = async (reviewId, action) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/postreviews/review/${reviewId}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  return (
    <div className="p-6">
            ...
            <div className="flex justify-between">
              <div>
                <div className="font-semibold">
                  {item.originalFileName || item.filePath}
                </div>
                  ...
                  (item.filePath.endsWith(".jpg") ||
                    ...
                    <div className="mt-2">
                      <img
                        ...
                        alt={item.originalFileName || "uploaded"}
              ...
              <div className="space-x-2">
                <button
                  ...
                  onClick={() => takeAction(item._id, "approve")}
                >
                  Approve
                </button>
                <button
                  className="px-3 py-1 bg-yellow-500 text-white rounded"
                  onClick={() => takeAction(item._id, "quarantine")}
                >
                  Quarantine
                </button>
                <button
                  className="px-3 py-1 bg-red-600 text-white rounded"
                  onClick={() => takeAction(item._id, "reject")}
                >
                  Reject
                </button>
              </div>
      ...
      <h3 className="text-xl text-black font-semibold mt-6 mb-3">
        Pending Posts (Lost/Skill/Notes)
      </h3>
          ...
          <div key={p._id} className="p-3 bg-white rounded shadow">
            <div className="flex justify-between">
              <div>
                ...
                <div className="text-sm text-gray-600">
                  Submitted by:{" "}
                  {p.uploadedBy?.fullName || p.uploadedBy?.email}
                </div>
                <div className="text-sm text-gray-500">
                  Status: {p.status}
                </div>
              ...
              <div className="space-x-2">
                <button
                  className="px-3 py-1 bg-green-500 text-white rounded"
                  onClick={() => takePostAction(p._id, "approve")}
                >
                  Approve
                </button>
                <button
                  className="px-3 py-1 bg-red-600 text-white rounded"
                  onClick={() => takePostAction(p._id, "reject")}
                >
                  Reject
                </button>
              </div>
