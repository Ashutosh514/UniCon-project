...
function ExtractedPostSkillModal({ onPostSuccess, userId, userName, setterRef, setShowContentPolicy }) {
  ...
  const [contentAcceptedLocal, setContentAcceptedLocal] = useState(false);

  useEffect(() => {
    if (setterRef) {
      setterRef.current = setContentAcceptedLocal;
      return () => { if (setterRef.current === setContentAcceptedLocal) setterRef.current = null; };
    }
  }, [setContentAcceptedLocal, setterRef]);
...
const SkillExchange = () => {
  ...
  // PostSkillModal moved to top-level (ExtractedPostSkillModal)
  const [formData, setFormData] = useState({
    title: '',
    description: '',
    category: '',
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-6">
          ...
          <div className="flex justify-end gap-4 pt-4">
            ...
            <button
              ...
              disabled={isSubmitting || !contentAcceptedLocal}
            >
...
return (
  <div className="bg-gray-100 min-h-screen font-sans antialiased text-gray-100">
    ...
    {/* Modals */}
    {isPostModalOpen && <ExtractedPostSkillModal onPostSuccess={handlePostSuccess} userId={userId} userName={userName} setterRef={contentPolicySetterRef} setShowContentPolicy={setShowContentPolicy} />}
